...
export function SupportChatPage() {
  ...
  const acceptDroppedFile = (nextFile?: File) => {
    if (!nextFile) return
        ...
        {chat.messages.map((msg) => (
          <div key={msg.id} className={`flex ${!msg.is_from_moderator ? 'justify-end' : 'justify-start'}`}>
            <div className={`max-w-[80%] ${!msg.is_from_moderator ? 'bg-indigo-600 text-white' : 'bg-surface border border-slate-200 text-slate-800'} rounded-2xl px-4 py-3 shadow-sm`}>
              ...
              {!isClosed ? <button type="button" onClick={() => { setReplyingTo(msg.id); textareaRef.current?.focus() }} className={`mt-1 text-[10px] underline underline-offset-2 ${!msg.is_from_moderator ? 'text-indigo-100' : 'text-slate-400'}`}>Ответить</button> : null}
            </div>
          </div>
        ))}
      </div>
      ...
      {isClosed ? (
        ...
        <div className="sticky bottom-0 bg-surface rounded-b-xl border border-slate-200 border-t-0 p-3 shrink-0" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
          <form
            ...
            onDrop={(event) => { event.preventDefault(); acceptDroppedFile(event.dataTransfer.files?.[0]) }}
            className={`relative flex items-end gap-2 rounded-xl transition-colors ${isDraggingFile ? 'ring-2 ring-indigo-500 bg-indigo-50' : ''}`}
          >
            ...
            <label className="flex h-[38px] w-[38px] shrink-0 cursor-pointer items-center justify-center text-slate-400 transition-colors hover:text-indigo-600">
              ...
              <input ref={fileInputRef} type="file" accept={SUPPORT_FILE_ACCEPT} className="hidden" onChange={(e) => acceptDroppedFile(e.target.files?.[0])} />
            </label>

            <div className="flex-1 min-w-0">
              {replyingTo ? (
                <div className="mb-1 flex items-center justify-between rounded-lg bg-slate-100 px-2 py-1 text-[10px] text-slate-500">
                  <span className="truncate">Ответ на: {chat.messages.find((item) => item.id === replyingTo)?.text || 'вложение'}</span>
                  <button type="button" onClick={() => setReplyingTo(null)} className="ml-2">×</button>
                </div>
              ) : null}
              <textarea
                ref={textareaRef}
            ...
            <button type="submit" disabled={isSending} aria-label={sendStatus === 'error' ? 'Повторить отправку' : 'Отправить'} className={`inline-flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-lg text-white transition-colors disabled:opacity-60 ${sendStatus === 'error' ? 'bg-red-600 hover:bg-red-700' : 'bg-indigo-600 hover:bg-indigo-700'}`}>
